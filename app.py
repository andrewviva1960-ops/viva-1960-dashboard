...
async function loadData() {
  ...
  Plotly.newPlot('monthlyChart', [
    {type:'bar', name:'Net Sales', x:monthLabels, y:nsV, offsetgroup:'a', marker:{color:_CALM.palette[0],opacity:0.85}, text:nsV.map(v=>fmtShort(v)), textposition:'outside', textfont:{size:11,color:'#4A5568',family:'Inter'}, cliponaxis:false},
    {type:'bar', name:'Expenses', x:monthLabels, y:expV, offsetgroup:'b', marker:{color:_CALM.palette[3],opacity:0.85}, text:expV.map(v=>fmtShort(v)), textposition:'inside', textfont:{size:10,color:'#fff',family:'Inter'}, cliponaxis:false},
    {type:'bar', name:'COGS', x:monthLabels, y:cogsV, offsetgroup:'b', base:expV, marker:{color:_CALM.palette[1],opacity:0.85}, text:cogsV.map(v=>fmtShort(v)), textposition:'outside', textfont:{size:11,color:'#4A5568',family:'Inter'}, cliponaxis:false}
  ], _calmLayout({barmode:'group', bargap:0.3, bargroupgap:0.1, height:300, hovermode:'x unified',
...
async function loadPnlData() {
  ...
  // 1. Net Sales chart - Forecast: dotted pattern bar (dots only, no fill)
  ...
  Plotly.newPlot('pnlSalesChart', [
    ...
    {type:'bar', name:'Forecast', x:activeMonths, y:nsFct,
     marker:{color:'#ffffff', pattern:{shape:'.', solidity:0.8, fgcolor:'#473FE0', size:6}},
